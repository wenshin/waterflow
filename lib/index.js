...
import {setCommonPipelineMiddlewares, setCommonPipeMiddlewares} from './pipeline';
/**
 * Middleware 设计为可拦截
 *
...
export default class Pipeline {
   ...
   * @return {undefined}
   */
  static setPipelineMiddlewares(middlewares=[]) {
    setCommonPipelineMiddlewares(middlewares);
  }
   ...
   * @return {undefined}
   */
  static setPipeMiddlewares(middlewares=[]) {
    setCommonPipeMiddlewares(middlewares);
  }
  ...
  flow(data, settings={}) {
    ...
    pipelined = this.reduce(
      (_pipelined, pipe) => {
        let {type='', middlewares=[]} = pipe;
        let pipelineMethod = 'flow' + strUtil.capitalize(type);
        return _pipelined[pipelineMethod]({...pipe, middlewares});
      }, pipelined);
