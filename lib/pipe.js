...
export function execAsyncMapPipe(state) {
  ...
  return {
    ...state,
    promise: promiseAll(outputState.value, state)
  };
}
...
export function execMapPipe(inputState, isAsync=false) {
  ...
  iterKeys.map((key, index) => {
    ...
    cpPipe.handle = data => handle(data, index, breakPipeline.bind(outputState));
 ...
 * @param  {Iterator|Object}  promises
 ...
 * @return {Promise}
 */
function promiseAll(promises, state, breakImmediately=false) {
  let isSkip = false;
  let isBreak = false;
  ...
  return new Promise((resolve, reject) => {
    ...
    let newState = value => {
      return {...state, break: isBreak, skip: isSkip, value};
    };

    let always = s => {
      isSkip = isSkip || s.skip;
      isBreak = isBreak || s.break;
